...
//(cd app && HOME=~/.electron-gyp yarn rebuild --runtime=electron --target=11.0.0-beta.18 --disturl=https://electronjs.org/headers --build-from-source); gulp build
...
gulp.task('rebuild', gulp.series(async () => {
  ...
  for (let mod of MODULES_NEEDING_REBUILD) {
    ...
    await runAsync(`yarn rebuild ${mod} --runtime=electron --target=11.0.0-beta.18 --disturl=https://electronjs.org/headers --build-from-source`, {cwd, env, shell: true})
  }
  await runAsync(`yarn run build`, {shell: true})
}))